...
def initializeSprites(app):
    # Sprites:
    app.swimmer = PlayerSpritesheet(app, 'images/swimmer.png')
    app.swimmer.scaleImage(app, .7)
    app.swimmer.initializeIdleSpriteList()
    app.swimmer.initializeRunningSpriteList()
    app.swimmer.initializeAttackSpriteList()
    app.swimmer.initializePhysicalAttackSpriteList()
    app.fish = FishSpritesheet(app, 'images/fish.png')
    app.fish.scaleImage(app, 0.2)
    app.fish.initializeRunningSpriteList(0.2)
...
def timerFired(app):
    ...
    if app.swimmer.attacking:
        app.swimmer.incrementAttackingCounter()
    elif app.swimmer.isRunning:
        app.swimmer.incrementRunningCounter()
    elif app.swimmer.physicalAttacking:
        app.swimmer.incrementPhysicalAttackingCounter()
    else:
        app.swimmer.incrementIdleCounter()
    app.fish.incrementRunningCounter()

    if app.player.cx < app.width // 2:
        app.fish.flipSpriteSheet(app.fish.runningSprites)
        app.fish.flipped = True
    elif app.player.cx >= app.width // 2:
        app.fish.flipSpriteSheet(app.fish.runningSprites)
        app.fish.flipped = False
...
def drawPlayer(app, canvas):
    if app.swimmer.attacking:
        sprite = app.swimmer.attackSprites[app.swimmer.attackingCounter]
    elif app.swimmer.isRunning:
        sprite = app.swimmer.runningSprites[app.swimmer.runningCounter]
    elif app.swimmer.physicalAttacking:
        sprite = app.swimmer.physicalAttackSprites[app.swimmer.physicalAttackCounter]
    else:
        sprite = app.swimmer.idleSprites[app.swimmer.idleSpriteCounter]
...
def drawMonsters(app, canvas):
    for monster in app.currentRoom.monsters:
        if isinstance(monster, fishMonster):
            sprite = app.fish.runningSprites[app.fish.runningCounter]
...
def drawMap(app, canvas):
    global color
    if not app.displayingMap: return
    for room in app.rooms:
        row, col = room.cell
        if room.cell == (0, 0):
            color = 'white'
...
def drawStartScreen(app, canvas):
    ...
    canvas.create_text(app.width // 2, app.height // 4,
                       text='Razhaye Shenavar', font='Arial 36 bold', fill='white')
    buttonX0 = app.width // 2 - 60
    buttonY0 = app.height // 2 - 30
    buttonX1 = app.width // 2 + 60
    buttonY1 = app.height // 2 + 30
    ...
    canvas.create_text(app.width // 2 + 80, app.height // 2 + 40,
                       text='Setting', fill='yellow', font='Arial 28 bold')
    canvas.create_text(app.width // 2, app.height // 2.8,
                       text='created by', font='Arial 12 bold', fill='white')
    canvas.create_text(app.width // 2, app.height // 2.6,
                       text='Mahdie & Masoume', font='Arial 12 bold', fill='white')
